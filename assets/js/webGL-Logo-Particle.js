...
document.addEventListener('DOMContentLoaded', () => {
	...
	if (heroPContainer) {
		...
		const animate = () => {
			...
			if (logoMesh) {
				logoMesh.rotation.y = elapsedTime * 0.5; // Normal spinning
				logoMesh.position.y = 5 + Math.sin(elapsedTime * 2) * 2; // Floating up and down
			}
			// Camera orbiting based on scroll
			const scrollAngle = window.scrollY * 0.002;
			const radius = 50;
			const targetX = Math.sin(scrollAngle) * radius + (mouseX * 0.05);
			const targetZ = Math.cos(scrollAngle) * radius;
